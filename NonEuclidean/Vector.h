...
public:
    ...
    inline Vector3 NormalizedSafe() const { return (*this) / (Mag() + FLT_EPSILON); }
    inline float Angle(const Vector3& b) const { return std::acos(Normalized().Dot(b.Normalized())); }
    inline float AngleSafe(const Vector3& b) const { return std::acos(NormalizedSafe().Dot(b.NormalizedSafe())); }
    ...
    // Other
    inline bool IsNDC() const { return (x > -1.0f && x < 1.0f && y > -1.0f && y < 1.0f && z > -1.0f && z < 1.0f); }
...
public:
    ...
    {
        return Vector4(
            m[0] * b.x + m[1] * b.y + m[2] * b.z + m[3] * b.w, m[4] * b.x + m[5] * b.y + m[6] * b.z + m[7] * b.w,
            m[8] * b.x + m[9] * b.y + m[10] * b.z + m[11] * b.w, m[12] * b.x + m[13] * b.y + m[14] * b.z + m[15] * b.w);
    }
    Vector3 MulPoint(const Vector3& b) const
    {
        const Vector3 p(
            m[0] * b.x + m[1] * b.y + m[2] * b.z + m[3], m[4] * b.x + m[5] * b.y + m[6] * b.z + m[7],
    ...
    {
        return Vector3(
            m[0] * b.x + m[1] * b.y + m[2] * b.z, m[4] * b.x + m[5] * b.y + m[6] * b.z,
